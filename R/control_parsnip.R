...
#'
#' Options can be passed to the [fit.model_spec()] function that control the
#' output and computations
#'
#' @param verbosity An integer where a value of zero indicates that no
#'  messages or output should be shown when packages are loaded or when the
#'  model is fit. A value of 1 means that package loading is quiet but model
#'  fits can produce output to the screen (depending on if they contain their
#'  own `verbose`-type argument). Additionally, A value of 2 or more indicates
#'  that any output should be seen and that the execution time of the fit should
