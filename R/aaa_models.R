...
#'  that have expanded documentation, for specific model/engine combinations.
...
#' @details
#' The current `parsnip` documentation is _dynamic_. There is a part at the
#'  top of the Rd file that populates a list of engines that depends in what
#'  packages are loaded at the time that the man file is loaded. For example, if
#'  a side-package has a new engine for `linear_reg()`, the
#'  `parsnip::linear_reg()` help can show a link to a detailed help page in the
#'  side package.
#'
#' To enable this, the process is to
#'
...
#'  documentation as well as a directive
#'  "`@includeRmd man/rmd/{model}_{engine}.Rmd details`".
#'
#'   1. Copy the file in `parsnip` that is in `man/rmd/setup.Rmd` and put it in
#'  the same place in your package.
#'
