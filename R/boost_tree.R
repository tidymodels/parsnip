...
translate.boost_tree <- function(x, engine = x$engine, ...) {
  ...
  if (engine == "spark") {
    if (x$mode == "unknown") {
      cli::cli_abort(
        "For spark boosted trees models, the mode cannot be 'unknown' if the specification is to be translated."
      )
...
as_xgb_data <- function(x, y, validation = 0, weights = NULL, event_level = "first", ...) {
  ...
  if (is.factor(y)) {
    if (length(lvls) < 3) {
      ...
      if (event_level == "second") cli::cli_warn("{.arg event_level} can only be set for binary variables.")
      y <- as.numeric(y) - 1
    }
  }
