...
`naivebayes::naive_bayes()` fits a model that uses Bayes' theorem to
compute the probability of each class, given the predictor values.
...
when using the `naivebayes` engine.
...
if the argument `usepoisson = TRUE` is passed as an engine argument.
