...
`catboost::catboost.train()` creates a series of decision trees forming
an ensemble. Each tree depends on the results of previous trees. All
...
and supply `mtry` as a proportion directly. For example, `mtry = 0.5`
with `counts = FALSE` means 50% of predictors are considered at each
split.

#### Engine-Specific Parameters

CatBoost has a large number of engine parameters. The current list is
found at <https://catboost.ai/docs/en/references/training-parameters>.
