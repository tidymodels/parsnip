...
test_that("numeric x and y", {
  ...
  observed <- convert_form_to_xy_fit(mpg ~ ., data = mtcars)
  ...
  expect_equal(
    mtcars[1:6, -1],
    convert_form_to_xy_new(observed, newdata = head(mtcars))$x
  )
})
...
test_that("numeric x and y, weights", {
  expected <- lm(mpg ~ . -disp, data = mtcars, weights = disp, 
                 x = TRUE, y = TRUE)
  observed <- convert_form_to_xy_fit(mpg ~ . -disp, data = mtcars, 
                             weights = disp)
  expect_equal(format_x_for_test(expected$x), observed$x)
  expect_equivalent(mtcars$mpg, observed$y)
  expect_equal(expected$terms, observed$terms)
...
test_that("numeric x and y, offset", {
  ...
  new_obs <- convert_form_to_xy_new(observed, newdata = mtcars[1:6,])
...
test_that("numeric x and y, offset in-line", {
  expected <- lm(mpg ~ cyl + hp +  offset(log(disp)), data = mtcars,  
                 x = TRUE, y = TRUE)
  observed <- convert_form_to_xy_fit(mpg ~ cyl + hp +  offset(log(disp)), 
                             data = mtcars)
  expect_equal(format_x_for_test(expected$x), observed$x)
  expect_equivalent(mtcars$mpg, observed$y)
  expect_equal(expected$terms, observed$terms)
...
test_that("numeric x and y, no intercept", {
  expected <- lm(mpg ~ 0 + ., data = mtcars,  
                 x = TRUE, y = TRUE)
  observed <- convert_form_to_xy_fit(mpg ~ 0 + ., data = mtcars)
  expect_equal(format_x_for_test(expected$x), observed$x)
  expect_equivalent(mtcars$mpg, observed$y)
  expect_equal(expected$terms, observed$terms)
  expect_equal(expected$xlevels, observed$xlevels)
...
test_that("numeric x and y, inline functions", {
  expected <- lm(log(mpg) ~ hp + poly(wt, 3), data = mtcars,  
                 x = TRUE, y = TRUE)
  observed <- convert_form_to_xy_fit(log(mpg) ~ hp + poly(wt, 3), 
                             data = mtcars)
  expect_equal(format_x_for_test(expected$x), observed$x)
...
test_that("numeric y and mixed x", {
  ...
  observed <- convert_form_to_xy_fit(rate ~ ., data = Puromycin)
  expect_equal(format_x_for_test(expected$x), observed$x)
  ...
  expect_equal(
    ...
    convert_form_to_xy_new(observed, newdata = head(Puromycin))$x
  )
})

test_that("numeric y and mixed x, omit missing data", {
  expected <- lm(rate ~ ., data = Puromycin_miss, x = TRUE, y = TRUE)
  observed <- convert_form_to_xy_fit(rate ~ ., data = Puromycin_miss)
  expect_equal(format_x_for_test(expected$x), observed$x)
  expect_equivalent(
  ...
  expect_equal(
    ...
    convert_form_to_xy_new(observed, newdata = head(Puromycin_miss), 
                   na.action = na.omit)$x
  )
})
...
test_that("numeric y and mixed x, include missing data", {
  ...
  observed <- convert_form_to_xy_fit(rate ~ ., 
                             data = Puromycin_miss, 
                             na.action = na.pass)
...
test_that("numeric y and mixed x, no dummies", {
  expected <- model.frame(rate ~ ., data = Puromycin)
  observed <-
    convert_form_to_xy_fit(rate ~ ., data = Puromycin, indicators = FALSE)
  expect_equivalent(expected, observed$x)
  
  expect_equal(
    format_x_for_test(head(expected))[, -1],
    convert_form_to_xy_new(observed, newdata = head(Puromycin))$x
  )
})
...
test_that("numeric x and numeric multivariate y", {
  expected <- lm(cbind(mpg, disp) ~ ., data = mtcars, 
                 x = TRUE, y = TRUE)
  observed <- convert_form_to_xy_fit(cbind(mpg, disp) ~ ., data = mtcars)
  expect_equal(format_x_for_test(expected$x), observed$x)
...
test_that("numeric x and factor y", {
  ...
  observed <- convert_form_to_xy_fit(Species ~ ., data = iris)
  expect_equal(format_x_for_test(expected$x), observed$x)
  ...
  expect_equal(
    ...
    convert_form_to_xy_new(observed, newdata = head(iris))$x
  )
...
test_that("numeric x and y, matrix composition", {
  ...
  new_obs <- convert_form_to_xy_new(observed, newdata = head(mtcars),
                            composition = "matrix")
...
test_that("numeric x and multivariate y, matrix composition", {
  ...
  observed <- convert_form_to_xy_fit(cbind(mpg, cyl)  ~ ., data = mtcars, 
                             composition = "matrix")
  expect_equal(format_x_for_test(expected$x, df = FALSE), observed$x)
...
test_that("data frame x, vector y", {
  observed <- convert_xy_to_form_fit(mtcars[, -1], mtcars$mpg)
  ...
  expect_equal(
    mtcars[1:6, -1],
    convert_xy_to_form_new(observed, newdata = head(mtcars[,-1]))
  )
})


test_that("matrix x, vector y", {
  observed <- convert_xy_to_form_fit(as.matrix(mtcars[, -1]), mtcars$mpg)
  expected <- mtcars[, c(2:11, 1)]
  names(expected)[11] <- "..y"
  expect_equal(expected, observed$data)
  expect_equal(formula("..y ~ ."), observed$formula)
  ...
  expect_equal(
    mtcars[1:6, -1],
    convert_xy_to_form_new(observed, newdata = as.matrix(mtcars[1:6,-1]))
  )
})


test_that("data frame x, 1 col data frame y", {
  observed <- convert_xy_to_form_fit(mtcars[, -1], mtcars[, "mpg", drop = FALSE])
  expected <- mtcars[, c(2:11, 1)]
  expect_equal(expected, observed$data)
...
test_that("matrix x, 1 col data frame y", {
  observed <- convert_xy_to_form_fit(as.matrix(mtcars[, -1]), 
...
test_that("data frame x, 1 col matrix y", {
  observed <-
    convert_xy_to_form_fit(mtcars[,-1], as.matrix(mtcars[, "mpg", drop = FALSE]))
  expected <- mtcars[, c(2:11, 1)]
  expect_equal(expected, observed$data)
  expect_equal(formula("mpg ~ ."), observed$formula)
  expect_equal(names(mtcars)[-1], observed$x_var)
...
test_that("data frame x, 2 col data frame y", {
  observed <- convert_xy_to_form_fit(mtcars[, -(1:2)], mtcars[, 1:2])
...
test_that("matrix x, factor y", {
  observed <- convert_xy_to_form_fit(as.matrix(iris[, -5]), iris$Species)
  expected <- iris
  names(expected)[5] <- "..y"
  expect_equal(expected, observed$data)
  expect_equal(formula("..y ~ ."), observed$formula)
...
test_that("data frame x, factor y", {
  observed <- convert_xy_to_form_fit(iris[, -5], iris$Species)
  expected <- iris
  names(expected)[5] <- "..y"
...
test_that("bad args", {
  expect_error(
    convert_xy_to_form_fit(mtcars$disp, mtcars$mpg)
  )
  expect_error(
    convert_xy_to_form_fit(mtcars[, 1:3], mtcars[, 2:5])
  )
})
