...
test_that('engine arguments', {
  xgboost_print <- boost_tree(mode = "regression", print_every_n = 10L)
  ...
  C5.0_rules <- boost_tree(mode = "classification", rules = TRUE)
...
test_that('updating', {
  expr1     <- boost_tree(             verbose = 0)
  expr1_exp <- boost_tree(trees = 10,  verbose = 0)

  expr2     <- boost_tree(trees = varying())
  expr2_exp <- boost_tree(trees = varying(), verbose = 0)

  expr3     <- boost_tree(trees = 1, sample_size = varying())
  expr3_exp <- boost_tree(trees = 1)

  expr4     <- boost_tree(trees = 10, rules = TRUE)
  expr4_exp <- boost_tree(trees = 10, rules = TRUE, earlyStopping = TRUE)

  expr5     <- boost_tree(trees = 1, rules = TRUE, earlyStopping = TRUE)

  expect_equal(update(expr1, trees = 10), expr1_exp)
  expect_equal(update(expr2, verbose = 0), expr2_exp)
  expect_equal(update(expr3, trees = 1, fresh = TRUE), expr3_exp)
  expect_equal(update(expr4, rules = TRUE, earlyStopping = TRUE), expr4_exp)
  expect_equal(update(expr5, rules = TRUE), expr5)
