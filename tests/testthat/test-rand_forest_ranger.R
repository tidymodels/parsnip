...
test_that('ranger classification intervals', {
  ...
  rgr_pred <- predict(
    extract_fit_engine(lc_fit),
    data = tail(lending_club)
  )$predictions
  expect_snapshot(
    rgr_se <- predict(
      extract_fit_engine(lc_fit),
