...
1. The ability to make predictions at multiple penalty values with a single model object. 
...
* When `predict()` is used on the `parsnip` model fit without specifying the `penalty` value, the saved value of the penalty is used. 

* To make predictions at multiple values along the path, use the `multi_predict()` function. Multiple predictions can be made simultaneously at different penalty values. 
...
You can modify the penalties contained in the path by using the [glmnet::glmnet()] arguments `nlambda` and `lambda.min.ratio`. The latter is discussed more below. These arguments would be passed when `set_engine()` is called. 
...
However, we have noticed that using the argument value `lambda.min.ratio = 0` in these fits might help achieve the correct results. However, consult the [glmnet::glmnet()] function documentation to read about the potential effects of using this value. 
