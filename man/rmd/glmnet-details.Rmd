...
In the discussion below, `linear_reg()` is used. The information is true for all \pkg{parsnip} models that have a `"glmnet"` engine. 

## Fitting and predicting using \pkg{parsnip}
...
Internally, the value of `penalty = 1` is saved in the \pkg{parsnip} object and no value is set for `lambda`. This enables the full path to be fit by `glmnet()`. See the section below about setting the path. 
...
\pkg{parsnip} models that use a `glmnet` engine can use a special optional argument called `path_values`. This is _not_ an argument to [glmnet::glmnet()]; it is used by \pkg{parsnip} to independently set the path. 
...
**tl;dr** `tidy()` on a `glmnet` model produced by \pkg{parsnip} gives the coefficients for the value given by `penalty`. 

When \pkg{parsnip} makes a model, it gives it an extra class. Use the `tidy()` method on the object, it produces coefficients for the penalty that was originally requested: 
