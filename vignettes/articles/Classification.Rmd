...
nnet_fit <- mlp(
  mode = "classification", 
  epochs = 100, units = 5, dropout = 0.1, 
  others = list(verbose = 0, validation_split = .20)
) %>%
  parsnip::fit(Status ~ ., data = juice(credit_rec), engine = "keras")
...
test_results <- credit_test %>%
  ...
  as_tibble() %>%
  mutate(
    `nnet class` = predict_class(nnet_fit, newdata = test_normalized),
    `nnet prob`  = predict_classprob(nnet_fit, newdata = test_normalized) %>% pull(good)
  )
