...
h2o::h2o.automl defines an automated model training process and returns
a leaderboard of models with best performances.
...
- `max_runtime_secs` and `max_models`: controls the maximum running time
  and number of models to build in the automatic process.

- `exclude_algos` and `include_algos`: a character vector indicating the
